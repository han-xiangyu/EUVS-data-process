    ...
    for traversal_idx in merged_traversal_idxes:
        ...
        for idx, sample_token in enumerate(all_sample_tokens):
            ...
            if idx not in skip_indices: ########### Sample data in certain ratio ##############
                ...
                if traversal_idx == 2 and idx<520:
                    continue
                elif traversal_idx == 86 and 103<idx<244:
                    continue
                elif traversal_idx == 36 and 100<idx<300:
                    continue
                # Get ego poses
                ...
                for sensor_channel in sensors:
                    ...
                    if len(images_list) == len(sensors):
                        ...
                        print(f"Image shapes do not match for frame {idx}, skipping.")
            else:
...
def main():
    ...
    nusc = NuScenes(version='v1.1', dataroot=f'./location_15/15', verbose=True)

    # # case 1 cross 1 straight and right strait
    # output_dir = '/home/xiangyu/Ultra/MARS_multitraversal/processed_data/loc_15_case_0_trainset_testset_incremental/testset'
    # # trainset_idxes = [2,69,78,82,83] # front all, sample rate 4/9 all cams
    # trainset_idxes = []
    # testset_idxes = [36] # front all, sample rate 1/4
    # train_sample_ratio = 4/9
    # test_sample_ratio = 1/4
    # train_sensors = [
    #         'CAM_FRONT_CENTER',
    #         'CAM_FRONT_LEFT',
    #         'CAM_FRONT_RIGHT',
    ...
    # case 1 cross 1 straight and right strait
    output_dir = '/home/xiangyu/Ultra/MARS_multitraversal/processed_data/loc_15_case_2_T_cross'
    trainset_idxes = [16,18,25,27] # front all, sample rate 1/3 all cams
    testset_idxes = [36] # front all, sample rate 1/4
...
#         lidar_output_dir_mike = os.path.join(output_dir, 'lidar')
#         os.makedirs(lidar_output_dir_mike, exist_ok=True)
#         test_txt_path = os.path.join(output_dir,'test_set.txt')
#         num_files = len(all_sample_tokens)
#         print("The number of samples in traversal ",traversal_idx," is: ",num_files)
#         skip_indices = np.linspace(0, num_files - 1, int(num_files * (1 - sample_ratio)), dtype=int)
...
#                 input_file, _, _ = nusc.get_sample_data(lidar_token)
#                 lidar_name = f"trav_{traversal_idx}_lidar_{img_idx}.pcd.bin"
#                 output_file = os.path.join(lidar_output_dir_mike, lidar_name)  # 保存为新的文件
#                 # 打开原始文件进行读取，并将其写入新文件
#                 with open(input_file, 'rb') as f_in:
#                     data = f_in.read()
#                 with open(output_file, 'wb') as f_out:
...
#     output_dir = './location_15/downsampled_multitraversal/case_1_cross_lidar_test'
#     # traversal_idxes = list(range(0, 51))

#     # case 1 cross 1 straight and right strait
#     traversal_idxes = [2,69,78,82,83] # front all, sample rate 4/9 all cams
#     # traversal_idxes = [] # front all, sample rate 4/9 all cams
#     testset_idxes = [79,86] # front all, sample rate 3/5 (front only)
#     # testset_idxes = []

#     # # case 2 cross 2 straight and left strait
#     # traversal_idxes = [2,69,78,82,83] # front all, sample rate 4/9 all cams
#     # testset_idxes = [36] # front all, sample rate 1/4 all cams
