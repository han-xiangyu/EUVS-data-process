    ...
    for traversal_idx in merged_traversal_idxes:
        ...
        for idx, sample_token in enumerate(all_sample_tokens):
                ...
                for sensor_channel in sensors:
                    if sensor_channel == 'CAM_FRONT_CENTER':
                        sample_ratio = 2/5
                        TESTSET = False
                    else:
                        sample_ratio = 1/20
...
def main():
    ...
    nusc = NuScenes(version='v1.1', dataroot=f'./location_06_10days/6', verbose=True)

    output_dir = './location_06_10days/downsampled_multitraversal/test_lidar'
    # traversal_idxes = list(range(0, 51))

    # right lane change
    traversal_idxes=[11,13,14,18,31,33,34,35] # location 06 10days front only
    testset_idxes = [] # location 06 10days front side testset
