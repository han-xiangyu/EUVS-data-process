...
import json
from project_pc_to_image import generate_depth_gt_image
...
def process_data(nusc, output_dir, trainset_idxes, testset_idxes,  train_sensors, test_sensors, train_sample_ratio, test_sample_ratio):
    ...
    for traversal_idx in merged_traversal_idxes:
        ...
        depth_gt_output_dir = os.path.join(output_dir, 'depth_gt')
        os.makedirs(depth_gt_output_dir, exist_ok=True)
        ...
        for idx, sample_token in enumerate(all_sample_tokens):
            ...
            if idx not in skip_indices: ########### Sample data in certain ratio ##############
                ...
                for sensor_channel in sensors:
                    ...
                    # # ---------------------Generate depth gt img ------------------------------
                    # seg_mask_folder = os.path.join(output_dir, 'seg_mask')
                    # sky_mask_folder = os.path.join(output_dir, 'sky_mask')
                    # pcd_path = input_file
                    # name = os.path.basename(image_name).split(".")[0]
                    # seg_mask_path = os.path.join(seg_mask_folder, name) + '.npy'
                    # sky_mask_path = os.path.join(sky_mask_folder, name) + '.npy'
                    # depth_gt_output_path = os.path.join(depth_gt_output_dir, name) + '.npy'
                    # generate_depth_gt_image(nusc, img_dir=image_path, pcd_dir=pcd_path, cam_sample=camera_data, lidar_sample=sd_record_lid,seg_mask_path=seg_mask_path,sky_mask_path=sky_mask_path,output_path=depth_gt_output_path)
                    # # -------------------------------------------------------------------------
...
def main():
    ...
    nusc = NuScenes(version='v1.1', dataroot=f'/home/xiangyu/Ultra/MARS_multitraversal/location_43/43', verbose=True)

    # case 1 T_cross
    output_dir = '/home/xiangyu/Common/loc_43_case_1_T_cross'
    trainset_idxes = [14,34,35,42] # front all, sample rate 1/5
    # trainset_idxes = []
    testset_idxes = [39]  # front only, sample rate 1/3
    train_sample_ratio = 3/10
    ...
    # # case 2 T_cross all cams
    # output_dir = '/home/xiangyu/Common/loc_43_case_1_all_cams/train'
    # trainset_idxes = [14,34,35,42] # front all, sample rate 1/5
    # # trainset_idxes = []
    # # testset_idxes = [39]  # front only, sample rate 1/3
    # testset_idxes = []  # front only, sample rate 1/3
    # train_sample_ratio = 3/20
    # test_sample_ratio = 1/3
    # train_sensors = [
    #         'CAM_FRONT_CENTER',
    #         'CAM_FRONT_LEFT',
    ...
    # # case 2 T_cross single traversal train
    # output_dir = '/home/xiangyu/Common/loc_43_case_2_single_traversal/train'
    # trainset_idxes = [35] # front all, sample rate 1/5
    # # trainset_idxes = []
    # # testset_idxes = [39]  # front only, sample rate 1/3
    # testset_idxes = []  # front only, sample rate 1/3
    ...
    # Extra case MemoGaussian
    output_dir = '/home/xiangyu/Common/MemoGaussian/loc43'
    trainset_idxes = [34,35,42,9,12,13] # front all, lots of cars 9,12,13
    # trainset_idxes = []
    testset_idxes = []  
    train_sample_ratio = 1/4
    test_sample_ratio = 1
    train_sensors = [
            'CAM_FRONT_CENTER',
            'CAM_FRONT_LEFT',
...
#     output_dir = './location_62/downsampled_multitraversal/cross'


#     # # middle lane change
#     # traversal_idxes = [5,28,30,41,44,45] # front all, sample rate 1/6
#     # testset_idxes = [6,7,11,13,23]  # front only, sample rate 1/3

#     # # right lane change
#     # traversal_idxes = [5,28,30,41,44,45] # front all, sample rate 1/6
#     # testset_idxes = [22]  # front all, sample rate 1/3

#     # cross
#     traversal_idxes = [6,7,11,13,23] # front all, sample rate 5/12
#     testset_idxes = [47] # front all, sample rate 1/2
