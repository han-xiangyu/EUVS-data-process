...
def add_label_to_image(image, label):
    ...
    font_path = "/home/xiangyu/Ultra/Download/optima/OPTIMA.TTF"  # 根据实际字体路径修改
    try:
...
def parse_folder(folder_path):
    """
    解析GT文件夹，用正则解析 trav_(\d+)_channel_(\d+)_img_(\d+).png 格式的文件
    返回结构: parsed[trav_id][frame_id][channel_id] = path
    """
    pattern = r'trav_(\d+)_channel_(\d+)_img_(\d+)\.png'
...
if __name__ == "__main__":
    model_folder = "/home/xiangyu/Common/EUVS_data/Level_3_MARS/loc_43_case_1_T_cross/models"
    baseline_labels = ["Ground truth", "3DGS", "3DCS"]

    train_paths = [
        os.path.join(model_folder, "3DGS/train/ours_30000/gt"),
        os.path.join(model_folder, "3DGS/train/ours_30000/renders"),
        os.path.join(model_folder, "3DCS/train/ours_30000/renders")
    ]
    ...
    test_paths = [
        ...
        os.path.join(model_folder, "3DCS/test/ours_30000/renders")
    ]

    output_video_path = os.path.join(model_folder, "test_set_video.mp4")
    generate_video_from_folders(test_paths, output_video_path, baseline_labels=baseline_labels, fps=10)
